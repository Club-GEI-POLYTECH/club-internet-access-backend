...
export class TicketsController {
    ...
    schema: {
      example: {
        ticket: {
          ...
          profile: 'TEST',
          status: 'reserved',
          ticketType: { id: 'type-uuid', name: '24 heures', price: 5000 },
        },
        payment: {
          id: 'payment-uuid',
          amount: 5000,
  ...
  @ApiOperation({
    summary:
      'Prévisualise les types détectés dans un CSV d’import (24h/7j/30j, prix recommandés, type existant ou à créer)',
  })
  @ApiConsumes('multipart/form-data')
  @ApiBody({
    schema: {
      ...
      properties: {
        file: {
          ...
          format: 'binary',
          description:
            'Fichier CSV Mikhmon. Le backend regroupe par durée (24h/7j/30j) et recommande le type avant import.',
        },
      },
    },
  })
  @UseInterceptors(FileInterceptor('file'))
  @ApiResponse({ status: 200, description: 'Prévisualisation des types détectés' })
  @ApiResponse({ status: 400, description: 'Fichier invalide' })
  @ApiResponse({ status: 401, description: 'Non autorisé' })
  async importRecommendations(
    ...
    this.logger.log(
      `POST /tickets/admin/import/recommendations filename=${file?.originalname} size=${file?.size}`,
    );
    const csvContent = file.buffer.toString('utf-8');
    return await this.ticketsService.getImportTypeRecommendations(csvContent);
  }
